 ...
 * MERGED FROM: extract.js + normalizer.js
 */

import { decodeUrl, cleanJsonString, extractAll } from '../../core/parser/regex.extractor.js';
import { decodeHtmlEntities } from '../../core/parser/html.parser.js';
import {
...
function isValidMediaUrl(url, type = 'video') {
  if (!url || url.length < VALID_MEDIA_PATTERNS.minLength) return false;
  if (/<|>/.test(url)) return false;
  return VALID_MEDIA_PATTERNS[type].test(url);
}
...
export function extractVideoUrls(fragment, seenUrls, thumbnail, index) {
  ...
  const add = (quality, url) => {
    if (!url.includes('.mp4') && !isValidMediaUrl(url, 'video')) return;
    if (seenUrls.has(url) || found.has(quality)) return;
    seenUrls.add(url);
    found.add(quality);
    formats.push({ index: index, quality: quality, type: 'video', url: url, thumbnail: thumbnail });
  };
  ...
  if (dashMatches.length > 0) {
    const dashVideos = dashMatches
      .map(function(m) { return { height: parseInt(m[1]), url: decodeUrl(m[2]) }; })
      .filter(function(v) { return v.height >= 360; })
      .sort(function(a, b) { return b.height - a.height; });

    const hd = dashVideos.find(function(v) { return v.height >= 720; });
    const sd = dashVideos.find(function(v) { return v.height < 720 && v.height >= 360; });
...
export function extractStories(html, seenUrls) {
  seenUrls = seenUrls || new Set();
  const formats = [];
  let itemIndex = 0;
  ...
  for (let i = 0; i < videoMatches.length; i++) {
    ...
    if (!seenUrls.has(url)) {
      seenUrls.add(url);
      videoPairs.push({ url: url, isHD: isHD });
    }
  }
  ...
  if (videoPairs.length === 0) {
    ...
    for (let i = 0; i < fallbackMatches.length; i++) {
      ...
      if (!seenUrls.has(url)) {
        seenUrls.add(url);
        videoPairs.push({ url: url, isHD: /720p|1080p|_hd/.test(url) });
      }
    }
  }
  ...
  const hasHD = videoPairs.some(function(v) { return v.isHD; });
  const hasSD = videoPairs.some(function(v) { return !v.isHD; });

  if (hasHD && hasSD) {
    const count = Math.ceil(videoPairs.length / 2);
    for (let i = 0; i < count; i++) {
      const pair = videoPairs.slice(i * 2, i * 2 + 2);
      const hd = pair.find(function(v) { return v.isHD; });
      const sd = pair.find(function(v) { return !v.isHD; });
...
export function extractImageUrls(html, decoded, seenUrls, targetPostId) {
  seenUrls = seenUrls || new Set();
  const formats = [];
  const seenPaths = new Set();
  let itemIndex = 0;

  const add = function(imgUrl) {
  ...
  if (itemIndex === 0) {
    ...
    for (let i = 0; i < viewerMatches.length; i++) {
      const height = parseInt(viewerMatches[i][1]);
      const width = parseInt(viewerMatches[i][2]);
      const url = cleanJsonString(viewerMatches[i][3]);
      
      if (/scontent|fbcdn/.test(url) && height >= 400 && width >= 400) {
        if (/t39\.30808|t51\.82787/.test(url) && !/\/[ps]\d{2,3}x\d{2,3}\/|\/cp0\//.test(url)) {
          candidates.push({ url: url, size: height * width });
        }
      }
    }
...
export function extractMetadata(html, url) {
  const metadata = { author: null, description: null, thumbnail: null, uploadDate: null, engagement: {} };
  const isStory = CONTENT_TYPE_PATTERNS.story.test(url);

  const decode = function(s) {
    return s.replace(/\\u([\dA-Fa-f]{4})/g, function(_, h) {
  ...
  if (timeMatch) {
    metadata.uploadDate = new Date(parseInt(timeMatch[1]) * 1000).toISOString();
  }

  // Engagement parser
  const parseEngagement = function(s) {
    if (!s) return 0;
...
// ============================================================
// OUTPUT BUILDERS (from normalizer.js)
// ============================================================

function groupFormatsToItems(formats, defaultThumbnail) {
  ...
  for (let i = 0; i < formats.length; i++) {
    ...
    if (!itemsMap.has(idx)) {
      itemsMap.set(idx, {
        ...
        thumbnail: format.thumbnail || defaultThumbnail,
        sources: []
      });
    }
    ...
    const source = { quality: quality, url: format.url };
  ...
  const items = Array.from(itemsMap.values())
    ...
    .map(function(item) {
      item.sources.sort(function(a, b) {
        const order = { hd: 0, sd: 1, original: 2, audio: 3 };
...
export function deduplicateFormats(formats) {
  const seen = new Set();
...
export function buildExtractResult(data) {
  const formats = data.formats;
  const metadata = data.metadata;
  const urlContentType = data.contentType;
  const id = data.id;
  ...
  logger.media('facebook', {
    items: items.length,
    totalSources: items.reduce(function(sum, i) { return sum + i.sources.length; }, 0)
  });

  return result;
}

export function buildErrorResult(code, message) {
  return {
    success: false,
